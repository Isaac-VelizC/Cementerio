        ...
        <div class="card-body pt-4 p-3">
            <form action="{{ url('/admin/personals/'.$personalEdit->id.'/edit') }} " method="POST" role="form text-left">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="user.sector" class="form-control-label">{{ __('Sector de trabajo') }}</label>
                            <div class="@error('user.sector')border border-danger rounded-3 @enderror">
                                <select class="form-control" name="sector_id" id="sector" placeholder="Seleccionar">
                                    @foreach ($sector as $item)
                                        <option value="{{ $item->id }}" @if($item->id == old('sector_id', $personalEdit->sector_id)) selected @endif>{{ $item->sector }}</option>
                                    @endforeach
                                </select>
                                @error('sector')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
